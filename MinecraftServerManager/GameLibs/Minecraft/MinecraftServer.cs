...
namespace MinecraftServerManager.Minecraft
{
    public class MinecraftServer
    {
        ...
        private async Task CreateBackup(List<string> pathsToBackup, string destinationPath)
        {
            ...
            {
                foreach (var relativePath in pathsToBackup)
                {
                    foreach (var fileData in await GetFiles($"{MinecraftRootFolder}/{relativePath}"))
                    {
                        var relativeFileName = fileData.Name;
                        if (!fileNames.Contains(relativeFileName))
                        {
                            fileNames.Add(relativeFileName);
                            writer.Write(relativeFileName, new MemoryStream(fileData.Content), fileData.ModificationTime);
                        }
                    }
                }
            }
        ...
        {
            return Encoding.UTF8.GetString((await GetFiles(pathInContainer))[0].Content).ReplaceLineEndings();
        }
        ...
        internal async Task<List<FileData>> GetFiles(string path)
        {
            var files = new List<FileData>();
            GetArchiveFromContainerParameters parameters = new GetArchiveFromContainerParameters { Path = path };
            ...
            while (reader.MoveToNextEntry())
            {
                if (!reader.Entry.IsDirectory)
                {
                    ...
                    var fileData = new FileData
                    {
                        ...
                        ModificationTime = reader.Entry.LastModifiedTime
                    };
                    files.Add(fileData);
                }
            }
